...
// count the number of misses the CPU has made
var numCPUmisses = 0;
...
var processSpeech = function(transcript) {
  ...
  if (gameState.get('state') == 'setup') {
    ...
    if (gameState.isPlayerTurn()) {
      ...
      if (userSaid(transcript, ['hit','Hit', 'HIT', 'hit!','miss', 'Miss', 'MISS', 'sink', 'Sink', 'sunk', 'SINK','game over'])) {
        if ('hit' == transcript || 'Hit' == transcript || 'HIT' == transcript || 'hit!' == transcript) {
          response = 'hit';
        } else if ('miss' == transcript || 'Miss' == transcript) {
          response = 'miss';
        } else if ('sink' == transcript || 'sunk' == transcript || 'SINK' == transcript) {
          response = 'sunk';
        } else if ('game over' == transcript) {
          response = 'game over';
        } 
...
var registerCpuShot = function(playerResponse) {
  ...
  if (result.isGameOver) {
    if (playerResponse != 'game over') {
      generateSpeech('The game is over.');
    }
      generateSpeech('I won the game!');
  ...
  // Sunk ship
    ...
    if (playerResponse != 'sunk') {
      if (playerResponse == 'hit') {
        generateSpeech('A ship was sunk!');
      } else {
        generateSpeech('Stop lying. A ship was sunk.');
      }
    }
    var shipName = result.sunkShip.get('type');
    generateSpeech('Hurray!');
    numCPUmisses = 0;
  }
  // Hit or miss
  else {
    ...
    if (isHit) {
      if (playerResponse != 'hit') {
        generateSpeech('Stop lying. A ship was hit.');
      }
      generateSpeech('Awesome!');
      numCPUmisses = 0;
    } else {
      if (playerResponse != 'miss') {
        generateSpeech('Stop lying. That was a miss.');
      }
      numCPUmisses += 1;
      if (numCPUmisses > 3) {
        generateSpeech('Oh no! I need to be better at guessing.')
      }
      generateSpeech('Maybe next time');
    }
  }
